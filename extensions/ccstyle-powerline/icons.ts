...
export const THINKING_TEXT_NERD: Record<string, string> = {
  ...
  max: "\u{F06D} max",       // fire (maximum reasoning)
};
...
// ASCII/Unicode fallback icons — pure ASCII only, no obscure Unicode that may render as □
...
/* Detect Nerd Font support via env var, WT_SESSION, or TERM_PROGRAM.
 * Windows Terminal (WT_SESSION) often ships with Nerd Fonts or Cascadia Code PL. */
export function hasNerdFonts(): boolean {
  if (process.env.POWERLINE_NERD_FONTS === "1") return true;
  if (process.env.POWERLINE_NERD_FONTS === "0") return false;
  if (process.env.GHOSTTY_RESOURCES_DIR) return true;
  // Windows Terminal — many users have Nerd Fonts installed
