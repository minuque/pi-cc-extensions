...
import { colorThinking, getDefaultColors, invalidateThemeCache } from "./theme.ts";
...
export interface PowerlineController {
  getPreset(): StatusLinePreset;
  getPresets(): StatusLinePreset[];
  setPreset(value: string, ctx: any): { valid: boolean; persisted: boolean };
}

export default function powerlineFooter(pi: ExtensionAPI): PowerlineController {
  // Transitive extension modules can survive Pi /reload; never retain their
  // previous theme.json cache for the new Powerline instance.
  ...
  // Messages received while the chat viewport is scrolled away from the bottom.
  let pendingNewMessages = 0;
  // Fallback guard for runtimes that deliver the first assistant update before
  // the extension's message_start handler.
  ...
  pi.on("session_start", async (event, ctx) => {
    ...
    if (enabled && ctx.hasUI) {
      ...
      // Color-only changes do not alter visible text. Invalidate the editor and
      // request a full frame so /reload cannot retain the previous ANSI colors.
      currentEditor?.invalidate?.();
      resetLayoutCache();
      tuiRef?.requestRender(true);
      const renderGeneration = sessionGeneration;
      queueMicrotask(() => {
        if (sessionGeneration !== renderGeneration || currentCtx !== ctx) return;
        currentEditor?.invalidate?.();
        fixedEditorContainer?.invalidate?.();
        fixedStatusContainer?.invalidate?.();
        fixedWidgetContainerAbove?.invalidate?.();
        fixedWidgetContainerBelow?.invalidate?.();
        resetLayoutCache();
        tuiRef?.requestRender(true);
      });
      if (settings.quietStartup === true && (event.reason === "startup" || event.reason === "new")) {
  ...
  pi.on("session_compact", async (_event, ctx) => {
    currentCtx = ctx;
    // Discard the last pre-compaction usage immediately. Core context usage is
  ...
  function noteAssistantMessage(): void {
  ...
  pi.on("message_update", async (event, ctx) => {
    if (event.message?.role === "assistant") noteAssistantMessage();
  ...
  const applyPowerlinePreset = (preset: StatusLinePreset, ctx: any): boolean => {
    currentCtx = ctx;
    config.preset = preset;
    resetLayoutCache();
    if (enabled) setupCustomEditor(ctx);
    requestImmediateStatusRender({ deferDuringTyping: false });
    return writePowerlinePresetSetting(preset, ctx.cwd);
  };
  ...
  pi.registerCommand("powerline", {
    ...
    handler: async (args, ctx) => {
      ...
      if (preset) {
        if (applyPowerlinePreset(preset, ctx)) {
  ...
  function buildSegmentContext(ctx: any, theme: Theme): SegmentContext {
    ...
    const contextPercent = coreContextUsage
      ? coreContextUsage.contextPercent
      : (contextWindow > 0 ? (fallbackContextTokens / contextWindow) * 100 : 0);
  ...
  function renderNewMessagesLine(width: number, theme: Theme): string[] {
    if (!fixedEditorCompositor) {
      pendingNewMessages = 0;
      return [];
    }
    // At the live bottom there is nothing to jump to, so never render a
    // redundant affordance. Clear stale counts defensively as well.
    if (fixedEditorCompositor.isAtRootBottom()) {
      pendingNewMessages = 0;
      return [];
    }

    const shortcut = formatShortcut(resolvedShortcuts.jumpChatBottom);
    const text = pendingNewMessages > 0
      ? `[ ↓ ${pendingNewMessages} new ${pendingNewMessages === 1 ? "message" : "messages"} · ${shortcut} ]`
      : `[ ↓ Back to bottom · ${shortcut} ]`;
    const styled = theme.fg("accent", truncateToWidth(text, width, "…"));
    const leftPad = Math.max(0, Math.floor((width - visibleWidth(styled)) / 2));
  ...
  /** Keep the jump affordance on the same row as pi's working/status indicator. */
  function mergeNewMessagesIntoStatusLine(
    statusLines: string[],
    width: number,
    theme: Theme,
  ): string[] {
    const buttonLine = renderNewMessagesLine(width, theme)[0];
    if (!buttonLine) return statusLines;

    const button = buttonLine.trimStart();
    const buttonStart = Math.max(0, Math.floor((width - visibleWidth(button)) / 2));
    const base = statusLines[0] ?? "";
    const gap = Math.max(1, buttonStart - visibleWidth(base));
    const combined = truncateToWidth(`${base}${" ".repeat(gap)}${button}`, width, "…");
    return [combined, ...statusLines.slice(1)];
  }
  ...
  function installFixedEditorCompositor(ctx: any, tui: any) {
    ...
    const stripAnsiForToolClick = (value: string): string => value
      .replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, "")
      .replace(/\x1b\[[0-9;?]*[ -/]*[@-~]/g, "")
      .replace(/\s+/g, " ")
      .trim();
    const expandableRootComponents: any[] = [];
    const nativeCompactionKeys = new WeakMap<object, string>();
    let nextNativeCompactionKey = 1;
    const isNativeCompactionComponent = (node: any): boolean =>
      node?.constructor?.name === "CompactionSummaryMessageComponent"
      && typeof node.setExpanded === "function";
    const collectExpandableRootComponents = (node: any, seen = new Set<any>()): void => {
      if (!node || typeof node !== "object" || seen.has(node)) return;
      seen.add(node);
      if (
        (typeof node.toolCallId === "string" || isNativeCompactionComponent(node))
        && typeof node.setExpanded === "function"
      ) {
        expandableRootComponents.push(node);
      }
      if (Array.isArray(node.children)) {
        for (const child of node.children) collectExpandableRootComponents(child, seen);
      }
    };
    const rootComponentIsLocallyExpanded = (component: any): boolean => {
      if (isNativeCompactionComponent(component)) return Boolean(component.expanded);
      const state = component.rendererState;
      return typeof state?.ccstyleToolExpanded === "boolean"
        ? state.ccstyleToolExpanded
        : Boolean(component.expanded);
    };
    const expandableRootComponentKey = (component: any): string => {
      if (typeof component.toolCallId === "string") return component.toolCallId;
      let key = nativeCompactionKeys.get(component);
      if (!key) {
        key = `compaction:${nextNativeCompactionKey++}`;
        nativeCompactionKeys.set(component, key);
      }
      return key;
    };
    const findExpandableRootComponentAtText = (rootLine: number, text: string): any | null => {
      const clicked = stripAnsiForToolClick(text);
      if (!clicked) return null;

      // Repeated tool rows can have identical text. Match the same occurrence
      // in root render order instead of always toggling the first one.
      const rootLines = ((compositor as any)?.rootLines ?? []) as string[];
      const clickedOccurrence = rootLines
        .slice(0, Math.max(0, rootLine) + 1)
        .filter((line) => stripAnsiForToolClick(line) === clicked)
        .length;
      let matchingOccurrence = 0;
      const width = Math.max(1, tui.terminal.columns || 80);
      expandableRootComponents.length = 0;
      collectExpandableRootComponents(tui);
      for (const component of expandableRootComponents) {
        const expanded = rootComponentIsLocallyExpanded(component);
        // A collapsed tool result is clickable only when it explicitly advertises
        // expansion. Once open, every non-empty line in its panel is clickable.
        if (!expanded && !/(?:to expand|\/ click)/.test(clicked)) continue;
        ...
        const matchingLines = renderedLines.filter((line) => {
          const rendered = stripAnsiForToolClick(line);
          return rendered === clicked || (clicked.length >= 8 && rendered.includes(clicked));
        }).length;
        if (matchingLines === 0) continue;
        matchingOccurrence += matchingLines;
        if (clickedOccurrence === 0 || clickedOccurrence <= matchingOccurrence) return component;
      }
      return null;
    };
    const toggleExpandableRootAtClick = (line: number, _col: number, text: string) => {
      const component = findExpandableRootComponentAtText(line, text);
      if (!component) return false;

      const expanded = rootComponentIsLocallyExpanded(component);
      if (isNativeCompactionComponent(component)) {
        // Preserve Pi's own inline compaction component and expansion rendering.
      ...
      // anchor; requesting here would paint an intermediate, shifted frame.
      return {
        handled: true as const,
        action: expanded ? "collapse" as const : "expand" as const,
    ...
    const removeThinkingBold = (line: string): string => line.replace(
      /\x1b\[([0-9;]*)m/g,
      (sequence, rawParams: string) => {
        const params = rawParams.split(";").filter((param) => !["1", "22"].includes(param));
        return params.length === 0 ? "" : `\x1b[${params.join(";")}m`;
      },
    );
    const hasItalicStyle = (line: string): boolean =>
    ...
    compositor = new TerminalSplitCompositor({
      ...
      jumpToBottomShortcut: resolvedShortcuts.jumpChatBottom,
      onJumpToBottom: () => {
        pendingNewMessages = 0;
        requestImmediateStatusRender({ deferDuringTyping: false });
      },
      ...
      onRootClick: toggleExpandableRootAtClick,
      onRootHover: isExpandableRootLine,
      decorateRootLines: decorateThinkingLines,
      onClusterClick: (_line, _col, text) => {
        const plain = text.replace(/\x1b\][^\x07]*(?:\x07|\x1b\\)/g, "").replace(/\x1b\[[0-9;?]*[ -/]*[@-~]/g, "");
        if (!fixedEditorCompositor || fixedEditorCompositor.isAtRootBottom()) return false;
        if (!plain.includes("new message") && !plain.includes("Back to bottom")) return false;
        jumpChatToBottom(currentCtx ?? ctx);
        return true;
      },
      ...
      renderCluster: (width, terminalRows) => {
        ...
        return renderFixedEditorCluster({
          width,
          terminalRows,
          statusLines: mergeNewMessagesIntoStatusLine(statusLines, width, theme),
  ...
  function setupWelcomeHeader(ctx: any) {
    ...
    ctx.ui.setHeader(() => {
      return {
        // Pi's quiet startup normally hides the native resource listing. Mark
        // this Claude Code Style header so that listing remains visible.
        showLoadedResourcesWhenQuiet: true,
        render(width: number): string[] {
  ...
  return {
    ...
    getPresets: () => Object.keys(PRESETS) as StatusLinePreset[],
    setPreset(value, ctx) {
      const preset = normalizePreset(value);
      if (!preset) return { valid: false, persisted: false };
      return { valid: true, persisted: applyPowerlinePreset(preset, ctx) };
    },
  };
}
