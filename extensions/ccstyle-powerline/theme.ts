...
// VS Code Modern Dark-inspired fallback defaults. Native pi themes take
// precedence for thinking-level colors when available.
const DEFAULT_COLORS: Required<ColorScheme> = {
  ...
  thinking: "#808080",
  thinkingOff: "#808080",
  thinkingMinimal: "#064675",
  thinkingLow: "#125489",
  thinkingMedium: "#1f5f99",
  thinkingHigh: "#3f87c5",
  thinkingXhigh: "#66a9dc",
  thinkingMax: "#8fc5f0",
  context: "#808080",
  contextWarn: "#dcdcaa",
  contextError: "#f14c4c",
...
/** Force the next render to reload theme.json, including after Pi /reload. */
...
const THINKING_SEMANTIC_COLORS: Record<string, SemanticColor> = {
  off: "thinkingOff",
...
/**
 * Apply the extension's monotonic blue scale so stronger thinking levels
 * become brighter and more prominent without switching to unrelated hues.
 */
export function colorThinking(
  ...
  const semantic = THINKING_SEMANTIC_COLORS[level] ?? "thinkingOff";
  return fg(theme, semantic, text, presetColors);
}
