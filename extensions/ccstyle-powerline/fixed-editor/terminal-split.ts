...
export interface RootClickResult {
  handled: true;
...
interface TerminalSplitCompositorOptions {
  ...
  jumpToBottomShortcut?: string;
  onJumpToBottom?: () => void;
  onCopySelection?: (text: string) => void;
  /** Handle a left click on a fixed-editor cluster line. Return true to consume it. */
  onClusterClick?: (line: number, col: number, text: string) => boolean;
  /** Handle a left click on a scrollable chat line. Return a result to preserve expansion anchors. */
  onRootClick?: (line: number, col: number, text: string) => boolean | RootClickResult;
  /** Return true when a scrollable chat line should show hover feedback. */
  onRootHover?: (line: number, col: number, text: string) => boolean;
  /** Apply stable presentation-only styling to the complete rendered transcript. */
  decorateRootLines?: (lines: string[]) => string[];
  /** Notify when manual scrolling reaches the newest output. */
  onReachRootBottom?: () => void;
}
...
function isMouseMotion(packet: SgrMousePacket): boolean {
  return packet.final === "M" && (packet.code & 32) !== 0;
}
...
export class TerminalSplitCompositor {
  ...
  private readonly jumpToBottomShortcut: string | null;
  private readonly onJumpToBottom: (() => void) | null;
  private readonly onCopySelection: ((text: string) => void) | null;
  private readonly onClusterClick: ((line: number, col: number, text: string) => boolean) | null;
  private readonly onRootClick: ((line: number, col: number, text: string) => boolean | RootClickResult) | null;
  private readonly onRootHover: ((line: number, col: number, text: string) => boolean) | null;
  private readonly decorateRootLines: ((lines: string[]) => string[]) | null;
  private readonly onReachRootBottom: (() => void) | null;
  ...
  private wroteDuringRenderPass = false;
  ...
  private hoveredRootLine: number | null = null;
  ...
  constructor(options: TerminalSplitCompositorOptions) {
    ...
    this.jumpToBottomShortcut = options.jumpToBottomShortcut ?? null;
    this.onJumpToBottom = options.onJumpToBottom ?? null;
    this.onCopySelection = options.onCopySelection ?? null;
    this.onClusterClick = options.onClusterClick ?? null;
    this.onRootClick = options.onRootClick ?? null;
    this.onRootHover = options.onRootHover ?? null;
    this.decorateRootLines = options.decorateRootLines ?? null;
    this.onReachRootBottom = options.onReachRootBottom ?? null;
  ...
  install(): void {
    ...
    if (this.originalDoRender) {
      this.tui.doRender = () => {
        ...
        this.wroteDuringRenderPass = false;
        this.renderPassCluster = null;
        try {
          this.originalDoRender?.();
          // Patched terminal writes already append the fixed cluster. Repainting
          // again here doubles terminal output and is a major source of flicker.
          if (!this.wroteDuringRenderPass) this.requestRepaint();
        } finally {
  ...
  jumpToRootBottom(): boolean {
    ...
    const moved = this.scrollOffset !== 0;
    this.clearSelection();
    this.lastLeftPress = null;
    this.scrollOffset = 0;
    this.pendingImageCleanup = true;

    // Always force a full repaint. After the first jump, some terminals can
    // move their physical viewport while our logical offset remains zero; the
    // old early return then made every later Ctrl+End a no-op.
  ...
  /** Whether the chat viewport is currently following the newest output. */
  isAtRootBottom(): boolean {
    return this.scrollOffset === 0;
  }

  /**
   * Drop viewport state whose line indexes belong to an obsolete transcript.
   * Compaction rebuilds historical message components, so retaining expansion
   * anchors or the previous line count makes later tool expansion drift.
   */
  resetTranscriptLayout(): void {
    if (this.disposed) return;

    this.toolExpansionStarts.clear();
    this.clearSelection();
    this.lastLeftPress = null;
  ...
  private refreshRootWindow(width: number): number {
    ...
    const lines = this.decorateRootLines?.(renderedLines) ?? renderedLines;
    this.rootLines = lines;
    if (!this.suppressRootGrowthAdjustment
      && this.scrollOffset > 0
      && this.lastRootLineCount > 0
  ...
  private handleInput(data: string): { consume?: boolean; data?: string } | undefined {
    ...
    // Handle this at the compositor input layer rather than relying on the
    // editor wrapper. Pi can replace/rebind the editor during a session, which
    // made the shortcut work once and then disappear on subsequent scrolls.
    if (
      !isKeyRelease(data)
      && this.jumpToBottomShortcut
      && matchesConfiguredShortcut(data, this.jumpToBottomShortcut)
    ) {
  ...
  private handleMousePacket(packet: SgrMousePacket): void {
    ...
    // Mode 1003 reports every mouse move. Hover can use the last rendered
    // viewport; rebuilding the whole root for each packet causes visible stalls.
    if (isMouseMotion(packet) && !this.selectionDragging) {
      const location = this.selectionLocationForPacket(packet);
      const rootText = location?.area === "root"
        ? this.visibleRootLines[location.point.line - this.visibleRootStart] ?? ""
        : "";
      const nextHoveredLine = location?.area === "root" && this.onRootHover?.(
        location.point.line,
        location.point.col,
        rootText,
      )
        ? location.point.line
        : null;
      if (nextHoveredLine !== this.hoveredRootLine) {
        this.hoveredRootLine = nextHoveredLine;
        this.requestRender();
      }
      return;
    }
    ...
    if (isMouseRelease(packet) && this.consumedClusterClick) {
      this.consumedClusterClick = false;
      this.clearSelection();
      this.lastLeftPress = null;
      return;
    }
    ...
    if (isLeftPress(packet)) {
      if (location.area === "root" && this.onRootClick) {
        const rootText = this.visibleRootLines[location.point.line - this.visibleRootStart] ?? "";
        // Keep the clicked row anchored while a tool opens downward. Each tool
        // remembers the viewport start from before expansion so collapsing it can
        // return to exactly that position, even after scrolling inside the panel.
        const viewportStartBeforeClick = this.visibleRootStart;
        this.suppressRootGrowthAdjustment = true;
        let clickResult: boolean | RootClickResult = false;
        try {
          clickResult = this.onRootClick(location.point.line, location.point.col, rootText);
          if (clickResult) {
            this.refreshRootWindow(Math.max(1, this.terminal.columns || 80));
            const detail = typeof clickResult === "object" ? clickResult : null;
            let targetStart = viewportStartBeforeClick;
            if (detail?.key && detail.action === "expand") {
              this.toolExpansionStarts.set(detail.key, viewportStartBeforeClick);
            } else if (detail?.key && detail.action === "collapse") {
              targetStart = this.toolExpansionStarts.get(detail.key) ?? viewportStartBeforeClick;
              this.toolExpansionStarts.delete(detail.key);
            }
            this.scrollOffset = Math.max(0, Math.min(
              this.rootLines.length - Math.max(1, this.visibleScrollableRows) - targetStart,
              this.maxScrollOffset,
            ));
            this.updateVisibleRootWindow();
          }
        } finally {
          this.suppressRootGrowthAdjustment = false;
        }
        if (clickResult) {
          this.clearSelection();
          this.lastLeftPress = null;
      ...
      if (location.area === "cluster" && this.onClusterClick?.(
        location.point.line,
        location.point.col,
        this.visibleClusterLines[location.point.line] ?? "",
      )) {
        this.clearSelection();
        this.lastLeftPress = null;
        this.consumedClusterClick = true;
  ...
  private renderHoverHighlight(line: string, lineIndex: number, area: SelectionArea): string {
    if (area !== "root" || this.hoveredRootLine !== lineIndex) return line;
    const plain = stripAnsi(line);
    if (!plain.trim()) return line;
    return `\x1b[1;97m${plain}\x1b[22;39m`;
  }
  ...
  private scrollBy(delta: number): void {
    ...
    this.scrollOffset = nextOffset;
    this.pendingImageCleanup = true;
    if (wasScrolled && nextOffset === 0) this.onReachRootBottom?.();
    // This writes the complete scrollable viewport synchronously; scheduling a
    // second full TUI render here only repaints the same frame again.
  ...
  private repaintScrollableViewport(width: number): void {
    ...
    for (let row = 0; row < scrollableRows; row++) {
      if (row > 0) buffer += "\r\n";
      buffer += clearLine();
      const hovered = this.renderHoverHighlight(this.visibleRootLines[row] ?? "", start + row, "root");
      buffer += sanitizeLine(this.renderSelectionHighlight(hovered, start + row, "root"), width);
    }
  ...
  private write(data: string): void {
    if (this.renderPassActive) this.wroteDuringRenderPass = true;
