...
import { fg, colorThinking, applyColor } from "./theme.ts";
...
const modelSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    // Keep the model and thinking level together without an extra dot marker:
    // `model [med]`. The thinking label uses pi's native level color.
    if (opts.showThinkingLevel !== false) {
      const level = ctx.thinkingLevel || "off";
      if (level !== "off") {
        const thinkingText = getThinkingText(level) ?? `[${level}]`;
        content += ` ${colorThinking(ctx.theme, level, thinkingText, ctx.colors)}`;
      }
    }
...
const thinkingSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    return {
      content: colorThinking(ctx.theme, level, content, ctx.colors),
      visible: true,
    };
  },
};
...
const contextPctSegment: StatusLineSegment = {
  ...
  render(ctx) {
    ...
    // A null percentage is pi's intentional post-compaction "unknown" state.
    // Show it neutrally until the next model response provides fresh usage.
    let content: string;
    if (pct !== null && pct > 90) {
